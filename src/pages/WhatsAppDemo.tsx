...
export default function WhatsAppDemo() {
  ...
  const handleSendMessage = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // Call Gemini from Frontend (Rule requirement)
      const { aiService } = await import('../services/aiService');
      let reply = await aiService.generateChatbotReply(newMsg.text, contextData.patientContext || 'Sin contexto disponible.');
      ...
      if (reply.includes('[ACTION:BOOK:')) {
        ...
        if (match) {
          ...
          await fetch('/api/whatsapp/book-from-chat', {
            method: 'POST',
            headers: { 'Content-Type': 'application/json' },
            body: JSON.stringify({
              clinicId: contextData.clinicId,
          ...
          reply = reply.replace(/\[ACTION:BOOK:.*?\]/, `✅ ¡Perfecto! He agendado tu cita para el ${new Date(requestedDate).toLocaleString('es-ES')}. ¡Te esperamos!`);
        }
      }
      
      setMessages(prev => [...prev, {
        id: Date.now().toString()+1,
        text: reply || 'Lo siento, ha habido un problema de conexión.',
        isMe: false,
        time: new Date().toLocaleTimeString('es-ES', {hour: '2-digit', minute:'2-digit'})
      }]);
    } catch (err) {
