        ...
        <div className="max-w-7xl mx-auto px-6 flex items-center justify-between">
          ...
          <div className="hidden lg:flex items-center gap-8">
                  ...
                  >
                    {services.map((item, i) => (
                      <div 
                        ...
                        className="px-5 py-2 hover:bg-gray-50 text-[13px] font-medium text-gray-500 hover:text-[#008477] transition-colors cursor-pointer"
                      >
                        {item.label}
                      </div>
                    ))}
            ...
            <button 
              ...
              className="text-[14px] font-semibold text-[#4f566b] hover:text-[#008477] transition-colors cursor-pointer"
            >
              Precios
            </button>
          </div>
            ...
            >
              <div className="px-6 py-8 space-y-6">
                <div className="space-y-4">
                  <p className="text-xs font-black uppercase tracking-widest text-gray-400">Servicios</p>
                  <div className="grid grid-cols-2 gap-4">
                    {services.map((item, i) => (
                      <div 
                        key={i} 
                        onClick={() => navigate(`/soluciones/${item.slug}`)}
                        className="text-[14px] font-bold text-gray-600 hover:text-[#008477]"
                      >
                        {item.label.replace('Software ', '')}
                      </div>
                    ))}
                  </div>
                </div>
                ...
                <div className="space-y-4 pt-4 border-t border-gray-50">
                  <p className="text-[14px] font-bold text-gray-600" onClick={() => handleNavClick('/', '#precios')}>Precios</p>
                  <p className="text-[14px] font-bold text-gray-600" onClick={() => navigate('/dashboard')}>Iniciar Sesión</p>
                </div>
                
                <button 
                  onClick={() => navigate('/dashboard')}
                  className="w-full py-4 bg-[#008477] text-white font-bold rounded-xl"
                >
                  Probar Gratis
                </button>
              </div>
